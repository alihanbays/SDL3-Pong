...
Box::Box() : collisionBox{0, 0, boxWidth, boxHeight}, xVelocity{0}, yVelocity{0}
{
}
...
void Box::AImove(Box &ball)
{
    ...
    else if (collisionBox.y + collisionBox.h > ScreenHeight)
    {
        collisionBox.y = ScreenHeight - collisionBox.h;
    }
}
...
void Box::render()
{
    SDL_FRect drawRect{static_cast<float>(collisionBox.x), static_cast<float>(collisionBox.y),
                       static_cast<float>(collisionBox.w), static_cast<float>(collisionBox.h)};
